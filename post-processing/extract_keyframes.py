...
def extract_frames_and_save_video(
            ...
            if adjusted_frame_index < cumulative_frames + frames:
                ...
                try:
                    ...
                    if not ret or frame is None:
                        print(f"Warning: Failed to read frame at index {relative_frame_index} from {video_path}. Skipping frame.")
                        cap.release()
                        continue

                    # Check for potential codec issues and ensure frame is valid
                    if cap.get(cv2.CAP_PROP_CODEC_PIXEL_FORMAT) == -1 or frame.size == 0:
                        print(f"Warning: Codec issue or empty frame detected in video {video_path}. Skipping frame.")
                        cap.release()
                        continue

                except Exception as e:
                    print(f"Exception caught when reading frame: {e}. Skipping frame.")
                    traceback.print_exc()
                    cap.release()
                    continue
    ...
    if frame_list:
        ...
        try:
            ...
            print(f"Error occurred while writing video: {e}")
            traceback.print_exc()

def process_task_concurrently(task_dir):
    try:
...
def main(root_dir):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(process_task_concurrently, task_dirs)
